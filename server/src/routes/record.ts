...
recordRoutes.route("/record").get(function (req: any, res: any) {
...
recordRoutes.route("/record/add").post(function (req: any, response: any) {
  let db_connect = dbo.getDb();
  let myobj = {
    name: req.body.name,
...
recordRoutes.route("/:id").delete((req: any, response: any) => {
  let db_connect = dbo.getDb();
  let myquery = { _id: ObjectId( req.params.id )};
